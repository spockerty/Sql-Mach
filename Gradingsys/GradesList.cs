    ...
    public partial class GradesList : Form
    {
        ...
        public void LoadStudentList()
        {
            string Query = "SELECT A.StudentID, " + 
	                           ...
	                           "B.Year, " +
                               "B.Section, " +
                               "(C.LastName + ', ' + C.FirstName) AS 'Teacher', " + 
                           ...
                           "FROM GradesTable A " + 
                           "INNER JOIN StudentInformationTable B " + 
                           "ON B.StudentID = A.StudentID " +
                           "INNER JOIN TeachersTable C " + 
                           "ON C.TeacherID = A.TeacherID " +
                           "INNER JOIN SubjectsTable D " +
                           "ON D.SubjectID = C.SubjectID " + 
                           "WHERE B.FirstName LIKE '%" + textBox1.Text + "%' " +
